// Safe Storage helper (prevents iframe SecurityError crashes)
    ...
    function updateAuthUI() {
      ...
      if (isAdmin) {
        ...
        // Lock branch filter dropdowns to this branch only
        const branchDropdowns = ['filter-dashboard-branch', 'filter-customer-branch'];
        branchDropdowns.forEach(dropId => {
          const el = document.getElementById(dropId);
          if (!el) return;
          let found = false;
          for (const opt of el.options) {
            if (opt.value === branchName) { el.value = branchName; found = true; break; }
          }
          el.dataset.forcedBranch = branchName;
          el.disabled = true;
        });
      }
    }
    ...
    function populateBranchDropdowns() {
      ...
      dropdowns.forEach(cfg => {
        ...
        // If branch user: force-select their branch and disable
        if (forcedBranch) {
          el.value = forcedBranch;
          el.disabled = (cfg.id === 'filter-dashboard-branch' || cfg.id === 'filter-customer-branch');
        } else {
          el.disabled = false;
    ...
    function resetCustomerFilters() {
      ...
      if (branchSel && (!AppState.auth || AppState.auth.role === 'admin')) {
        branchSel.value = 'ALL';
      }
    ...
    function renderCustomerList() {
      const branchSel = document.getElementById('filter-customer-branch');
      // Branch users: always filter to their own branch
      const isAdmin = AppState.auth && AppState.auth.role === 'admin';
      let selectedBranch = (branchSel && branchSel.value) ? branchSel.value : 'ALL';
      if (!isAdmin && AppState.auth && AppState.auth.branch) {
        selectedBranch = AppState.auth.branch;
        if (branchSel) branchSel.value = selectedBranch;
      }
      ...
      const customers = Object.values(AppState.customers || {});
      ...
      if (headerCustCount) headerCustCount.textContent = customers.length;
      ...
      let filtered = customers.filter(c => {
        ...
        if (selectedBranch !== 'ALL') {
          const hasBranch = (c.purchases || []).some(p => p.branch === selectedBranch);
    ...
    function openCustomerModal(encodedKey) {
      ...
      const cust = AppState.customers[key];
      if (!cust) {
      ...
      if (branchEl) branchEl.textContent = cust.primaryBranch || 'Main Branch';
      ...
      if (branchesEl) branchesEl.textContent = visitedBranches.length > 0 ? visitedBranches.join(', ') : (cust.primaryBranch || 'Main Branch');
      ...
      if (tableBody) {
        if (sortedPurchases.length === 0) {
          tableBody.innerHTML = '<tr><td colspan="6" class="text-center py-6 text-xs text-gray-500">No purchase records available.</td></tr>';
        } else {
    ...
    function exportCustomersToCSV() {
      ...
      a.click();
      URL.revokeObjectURL(url);
      showToast('Exported customer database to CSV!', 'success');
    }
